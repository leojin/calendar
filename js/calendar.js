    ...
    */
    ...
    var _calendar_debug = 1;

    var __calendar_todayInfo ;
    ...
    //打印log
    function _calendar_log(level,  msg) {
        if (_calendar_debug == 0 || _calendar_debug == level)
        if(window.console) {
    ...
    function drawdaylist(SY,SM){
        ...
        var day_seq = d.getDay();
        if(day_seq==0){
            day_seq = 7;
        }
        var f_day_s = Date.parse(d);
        var alldays = [];

        /*** 补充头部 start ***/
        for (var i = day_seq-1; i > 0 ; i--) { 

            var dd = new Date(f_day_s-86400*i*1000);
        ...
        var totaldays = (Date.parse(new Date(SY,SM-(-1),1))-f_day_s)/86400/1000;
       ...
       for (var i = 0; i < alldays.length; i++) {
            ...
            var lunar = calendar.solar2lunar(alldays[i].y,alldays[i].m,alldays[i].d);

            if(today==(alldays[i].y + "-" + alldays[i].m + "-" + alldays[i].d)){
                tmp+=" date-today";

                __calendar_todayInfo = lunar;
            }

            tmp+='"';


            tmp+= '><div  class="date-num" id="sd'+(i)+'">'+ alldays[i].d+'</div>';
            tmp+= '<div class="date-ch" id="ld'+(i)+'">'+ lunar.IDayCn +'</div></div>';

            divs[divs.length] = tmp;
            if(i>0 && (i+1)%7==0){
                boxs[boxs.length] = '<div class="box-date">' + divs.join('') + '</div>';
                divs = [];
            }
       }
    ...
    function drawdetail(y,m){
        ...
        s = __calendar_todayInfo.cDay;
        $('#right_detail_date_num').html(s);
    ...
    $('#addsomeevent').click(function(){
      ...
      _calendar_log(1,"documentHeight:"+docHeight);
      $("#overlay")
        .height(docHeight)
        .css({
          'opacity':0.8,
